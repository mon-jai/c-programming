...
void division(int number_1[SIZE], int number_2[SIZE], int result[SIZE]) {
  int no_of_bit[SIZE] = {0};
  no_of_bit[SIZE - 1] = 1;
  int result = {0};
  ...
  while (true) {
    ...
    multiplication(number_2, no_of_bit, temp);

    if (larger_than(temp, number_1)) {
      break;
    } else {
      int ten[SIZE] = {0};
      ten[SIZE - 2] = 1;
      multiplication(no_of_bit, ten, no_of_bit);
    }
  }
  ...
  while (true) {
    int to_be_substracted[SIZE] = {0};
        int zero[SIZE] = {0};
    int temp[SIZE] = {0};

    multiplication(number_2, no_of_bit, to_be_substracted);

    if (larger_than(temp, zero)) {
      break;
    } else {
      int ten[SIZE] = {0};
      ten[SIZE - 2] = 1;
